...
;(function($){
	...
	var config = $.strict.ios = function( options ){
		...
		info.browser = getBrowser( info, options );
		...
		window.addEventListener('orientationchange', function(e) {
			...
			if ( typeof options.orientationchange === "function" ) {
				options.orientationchange(e, me);
			}
		}, false );
	...
	function getSetting( options ) {
		...
		if ( typeof options.appTitle !== "undefined" && options.appTitle ) {
			var appTitle = {
	...
	function getBrowser( info, options ) {
		var ua = info.userAgent;
		var standalone = info.standalone;
		var browser;
		if ( ua.indexOf( "Intel Mac" ) !== -1 ) {
			...
			if ( ! ua.match( /iPhone|iPod|iPad/ ) ) {
				browser += "-PC";
			} else if ( options.isRejectSimulator && info.platform.indexOf( "Simulator" ) !== -1 ) {
				browser += "-Simulator";
			}
		} else {
			browser = "Other";
